...
- `Card asChild` + `<article>` vs `<article class={ ui.CardClasses(...) }>`.
...
| 5 | `<Card asChild variant="default" className="h-full hover:bg-muted/20"><article>…</article></Card>` | `<article class={ ui.CardClasses(ui.CardProps{Variant: "default", Class: "h-full hover:bg-muted/20"}) }>` | Same raised surface + hover utility on semantic root. |
