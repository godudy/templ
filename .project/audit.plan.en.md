...
| [~] | M5 | Task 1.3 modeled the fix as replicating a React hook (`useFrozenOpen`) per runtime | `components/sheet/sheet.tsx`:92 | A hook-shaped enforcement mechanism is, by construction, a fresh implementation per runtime (React hook, Svelte rune guard, Vue watcher, Go test). That's the same "edit N files by hand" failure mode called out for styling — it applies equally to behavioral contracts. | Wording fixed (part of **1.15**); **`validate-spec` lint implemented** — see **1.3** |
...
| [x] | 1.3 | **Sheet `Open` state — enforce at the contract level, not a per-runtime hook** | Medium | **Done.** Contract wording fixed in **1.15**. Added `validateSheetOpenContract` in `.validate/cmd/validate-spec/sheet_contract_validate.go` — flags forbidden Sheet Open teaching in `sheet.spec.md`, `docs/aria.md`, `docs/learn/03-sheet/README.md`, `docs/coming-from-shadcn.md`. Tests in `sheet_contract_validate_test.go`. |
